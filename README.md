...
I use local DNS in my homelab, but sometimes that resolver needs maintenance. This script gives me a quick way to switch my UniFi LAN/DHCP DNS settings between the UniFi gateway, a custom resolver such as AdGuard Home or Pi-hole, and a cloud-hosted resolver like Cloudflare, Google, or Quad9.

When maintenance is complete, I can switch the network back with one command.
...
- Public DNS providers like Cloudflare, Google, and Quad9 are handed directly to clients, which bypasses local LAN DNS records such as `*.localdomain`.
